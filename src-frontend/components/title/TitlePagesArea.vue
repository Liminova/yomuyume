...
function handleImageEmitInview(pageId: string): void {
	if (!pages.data.value) { return; }

	const chapterIdx = props.chapterIds.findIndex(chapter => chapter.id === props.chapterId);
	...
	setProgress.mutate({
		...
		page_id: pageId,
		percent: Math.round((chapterPercent + (pagePercent / chapterCount)) * 100),
	}, {
		onError: (error) => {
	...
	<div
		...
		<div
			...
			<Image
				...
				@in-view="() => { handleImageEmitInview(page.id); }" />
		</div>
	</div>
</template>
