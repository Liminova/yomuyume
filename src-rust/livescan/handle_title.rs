...
use crate::{
    livescan::cover_finder::{title_cover_finder, TitleCoverFinderError},
...
impl Scanner {
    pub async fn handle_title(
        ...
        // read <title_file_name>.toml
        let mut title_metadata = TitleMetadata::from(&scanned_title.path)?;

        // use file name as title if metadata not exists or is empty
        ...
        // insert/update title info to DB
        ...
        'update_cover: {
            ...
            .await
            .map_err(|e| match e {
                TitleCoverFinderError::NoCoverFound => "can't find cover".to_string(),
                TitleCoverFinderError::Io(e) => format!("can't read title file: {}", e),
                TitleCoverFinderError::Zip(e) => format!("can't read title file as zip: {}", e),
                TitleCoverFinderError::Path(e) => format!("can't strip temp dir prefix: {}", e),
            })?;

            title_metadata.set_cover(cover.image_path.to_string_lossy().to_string())?;
