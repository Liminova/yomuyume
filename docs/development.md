...
Since we rely on `HttpOnly` auth cookie for security, the APIs and frontend server must be served under the same origin (domain + port) and over HTTPS on the browser.
...
It's encouraged to use `sqlx::query!` and `sqlx::query_as!` macros to interact with SQLite since it
