...
<template>
	<div
		class="mb-7 mt-3 flex w-full flex-col gap-7 px-6 lg:mt-0 lg:pl-0 lg:pr-3"
	>
		<HomeRec />

		<div class="w-fit origin-left text-3xl font-bold transition-transform">
			Recently updated
		</div>
		<CarouselWrapper>
			<swiper-slide
				...
				:key="`a${title.id}`"
			>
				<TitleCard :key="`a${title.id}`" :title="title" />
			</swiper-slide>
		</CarouselWrapper>
		...
		<CarouselWrapper>
			<swiper-slide
				...
				:key="`b${title.id}`"
			>
				<TitleCard :key="`b${title.id}`" :title="title" />
			</swiper-slide>
		</CarouselWrapper>
	</div>
</template>
