...
export const GET_TITLE_PATH = (titleId: string) => `/api/content/title/${titleId}`;
export const GET_PAGES_PATH = (chapterId: string) => `/api/content/pages/${chapterId}`;
...
export const GET_COVER_FILE_PATH = (titleId: string) => `/api/file/cover/${titleId}`;
...
export async function NewYomuyumeRequest<T = void>(
	path: string,
	init?: RequestInit,
	queryParams?: Record<string, string | undefined | number | number[] | string[] | boolean>,
): Promise<T> {
